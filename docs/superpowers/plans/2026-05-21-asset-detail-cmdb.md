...
**Goal:** 将资产详情页从简单 `el-descriptions` 改为 CMDB 风格的 Tab 页签布局（基础信息 + 连接配置 + 预留 Tab）。

**Architecture:** 仅重写 `AssetDetailView.vue` 的模板和样式，后端 API 不变。使用 `el-tabs` 组件实现 4 个 Tab，其中前两个展示数据，后两个预留空状态。
...
| 操作 | 文件 | 职责 |
|------|------|------|
| Rewrite | `frontend/src/views/assets/AssetDetailView.vue` | CMDB Tab 布局 + Header + 编辑弹窗 |

---

### Task 1: 重写 AssetDetailView.vue
...
git commit -m "feat(asset-detail): redesign as CMDB-style tab layout"
```
