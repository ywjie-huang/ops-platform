<template>
  <div>
    <div class="page-header">
      <h2 class="page-title">工单协作</h2>
      <el-button type="primary" @click="showDialog()">+ 新增工单</el-button>
    </div>
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑工单' : '新增工单'" width="520px">
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="优先级">
          <el-select v-model="form.priority" style="width:100%"><el-option v-for="p in ['low','normal','high','critical']" :key="p" :label="p" :value="p" /></el-select>
        </el-form-item>
        <el-form-item label="状态">
          <el-select v-model="form.status" style="width:100%"><el-option v-for="s in ['open','in_progress','resolved','closed']" :key="s" :label="s" :value="s" /></el-select>
        </el-form-item>
...
async function fetchData() { loading.value = true; try { const res: any = await getTickets(filters); items.value = res.data.items } finally { loading.value = false } }
